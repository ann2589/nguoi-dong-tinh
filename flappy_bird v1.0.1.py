...
'''
    FLAPPY BIRD V1.0.1 (UPDATE CỦA BẢN V1.0.0) 
    CÁC SỬA ĐỔI:
    - Thay đổi cách sắp xếp dữ liệu class: ứng dụng class con
    - Thêm tính năng đổi frame khi hút (đang bị lỗi: cái phần hút không chạm đất) [Một phần của hard mode]
    - Thêm tính năng tốc độ game tăng dần khi thêm điểm => Hoàn thành normal mode 
'''
...
def is_collided(obja, objb): # KIỂM TRA VA CHẠM GIỮA 2 OBJ (MASK CỦA ẢNH)
    offset = (obja.x - objb.x, obja.y - objb.y) # Lấy vị trí tương đối của player so với ống cống

    return objb.hitbox.overlap(obja.hitbox, offset) is not None # True = có va chạm
...
def init_ground_list(my_list): # TẠO FRAME LIST CỦA MẶT ĐẤT
    ...
    ground1 = ground("ground.png", 0, HEIGHT - 100, WIDTH, 100, -5, 0) # ground frame 1
    ground2 = ground("ground.png", WIDTH, HEIGHT - 100, WIDTH, 100, -5, 0) # ground frame 2
...
def init_obstacle_list(my_list): # TẠO DANH SÁCH CHƯỚNG NGẠI VẬT: ỐNG CỐNG
    ...
    for i in range(number_of_obstacle): # tạo danh sách obstacle
        ...
        my_list.insert(0, obstacle("pipe_top.png", WIDTH * 1.5 + (obstacle_width + distance_between_obstacles) * i, y_top, obstacle_width, obstacle_height, obstacle_velocity_x, 0, True))
        my_list.insert(0, obstacle("pipe_bottom.png", WIDTH * 1.5 + (obstacle_width + distance_between_obstacles) * i, y_bottom, obstacle_width, obstacle_height, obstacle_velocity_x, 0, False))
...
def init_background_list(my_list): # TẠO DANH SÁCH FRAME PHẦN BACKGROUND
    background_stato = background("background-2-sky.png", 0, 0, WIDTH, HEIGHT, 0, 0) # Phần background đứng yên
    background_roto1 = background("background-2-buildings.png", 0, 0, WIDTH, HEIGHT - 100, -1, 0) # Phần background loop frame 1
    background_roto2 = background("background-2-buildings.png", WIDTH, 0, WIDTH, HEIGHT - 100, -1, 0) # Phần background loop frame 2
    my_list[:] = [background_stato, background_roto1, background_roto2]

def game_playing_easy():
    global score # liên kết score => cập nhật best_score
    ...
    for event in pygame.event.get(): # CHECK THAO TÁC
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                return ""
            elif event.key == pygame.K_SPACE or event.key == pygame.K_UP:  # CÁC THAO TÁC VỀ NHÂN VẬT
                current_character.jump()
...
# TẠO NHÂN VẬT
current_character = init_character("Perry-1.png", "Perry-3.png", "Perry-2.png", "Perry-4.png")
is_collecting = False
